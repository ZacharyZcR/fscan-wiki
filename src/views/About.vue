<template>
  <div class="max-w-7xl mx-auto px-6 transition-colors duration-300"
    ...
    <div class="mb-16 text-center py-12">
      <div class="inline-flex items-center justify-center p-4 rounded-full mb-6 bg-opacity-10"
        ...
        <Icon icon="mdi:shield-search" class="text-5xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      </div>
      <h1 class="text-5xl font-bold mb-4 transition-colors duration-300"
    ...
    <!-- 项目介绍卡片 - 更宽敞的布局 -->
    <div class="rounded-xl border shadow-xl overflow-hidden mb-16 transition-all duration-300 transform hover:shadow-2xl"
         :class="isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'">
      <div class="p-8 md:p-10">
        <div class="flex flex-col md:flex-row md:items-start">
          <div class="p-5 rounded-xl mr-8 md:mr-10 flex-shrink-0 mb-6 md:mb-0 inline-flex mx-auto md:mx-0"
               :class="isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600'">
            <Icon icon="mdi:information-outline" class="text-5xl" />
          </div>
          <div class="flex-1">
            <h2 class="text-3xl font-bold mb-6 transition-colors duration-300 text-center md:text-left"
                :class="isDark ? 'text-white' : 'text-gray-800'">项目介绍</h2>
            <div class="space-y-5 text-lg" :class="isDark ? 'text-gray-300' : 'text-gray-600'">
              ...
              <div v-if="repoInfo" class="mt-6 flex flex-wrap gap-4">
                <div class="inline-flex items-center px-4 py-2 rounded-xl text-base"
                     :class="isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'">
                  <Icon icon="mdi:star" class="mr-2 text-xl text-yellow-500" />
                  <span>{{ repoInfo.stargazers_count.toLocaleString() }} 星标</span>
                </div>
                <div class="inline-flex items-center px-4 py-2 rounded-xl text-base"
                     :class="isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'">
                  <Icon icon="mdi:source-fork" class="mr-2 text-xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
                  <span>{{ repoInfo.forks_count.toLocaleString() }} 复刻</span>
                </div>
                <div class="inline-flex items-center px-4 py-2 rounded-xl text-base"
                     :class="isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'">
                  <Icon icon="mdi:eye-outline" class="mr-2 text-xl" :class="isDark ? 'text-purple-400' : 'text-purple-600'" />
                ...
                <div class="inline-flex items-center px-4 py-2 rounded-xl text-base"
                  ...
                  <Icon icon="mdi:calendar-clock" class="mr-2 text-xl" />
              ...
              <div class="flex flex-wrap gap-5 mt-8">
                <a href="https://github.com/shadow1ng/fscan" target="_blank" rel="noopener noreferrer"
                   class="inline-flex items-center px-6 py-3 rounded-xl text-base font-medium transition-all duration-200 hover:scale-105"
                   :class="isDark ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'">
                  <Icon icon="mdi:github" class="mr-2 text-xl" />
                  访问 GitHub 仓库
                </a>
                <a href="https://github.com/shadow1ng/fscan/releases" target="_blank" rel="noopener noreferrer"
                  ...
                  <Icon icon="mdi:download" class="mr-2 text-xl" />
                  下载最新版本
                </a>
              </div>
            </div>
    ...
    <!-- 贡献者卡片 - 整行布局 -->
    <div class="rounded-xl border shadow-xl overflow-hidden mb-16 transition-all duration-300"
         :class="isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'">
      <div class="px-8 py-6 border-b flex items-center justify-between"
        ...
        <div class="flex items-center">
          ...
          <h2 class="font-bold text-2xl transition-colors duration-300"
              :class="isDark ? 'text-white' : 'text-gray-800'">项目贡献者</h2>
        </div>
        <span v-if="contributors.length > 0" class="px-3 py-1.5 text-sm rounded-full"
      ...
      <!-- 加载状态 -->
      <div v-if="loadingContributors" class="p-10 flex flex-col items-center justify-center">
        <div class="w-12 h-12 border-4 border-t-transparent border-blue-500 rounded-full animate-spin mb-6"></div>
        <p class="text-lg" :class="isDark ? 'text-gray-400' : 'text-gray-500'">加载贡献者信息...</p>
      </div>

      <!-- 错误提示 -->
      <div v-else-if="contributorsError" class="p-10 text-center">
        <div class="p-4 rounded-full bg-red-100 text-red-500 inline-flex items-center justify-center mb-6">
          <Icon icon="mdi:alert-circle" class="text-4xl" />
        </div>
        <p class="text-xl text-red-500 mb-4">无法加载贡献者信息</p>
        <p class="text-base" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{ contributorsError }}</p>
        <button @click="fetchContributors" class="mt-6 px-6 py-3 bg-blue-500 text-white rounded-xl text-base">
          重试
        </button>
      </div>
      ...
      <div v-else class="p-8">
        <!-- 主要贡献者部分 -->
        <div class="mb-10" v-if="contributors.length > 0">
          <h3 class="text-lg uppercase tracking-wider mb-6 opacity-70"
              :class="isDark ? 'text-gray-400' : 'text-gray-500'">主要贡献者</h3>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- 作者/首要贡献者 -->
            <div class="flex items-center p-5 rounded-xl transition-all duration-200 group"
                 :class="isDark ? 'bg-blue-900/10 hover:bg-blue-900/20' : 'bg-blue-50 hover:bg-blue-100'">
              <div class="relative">
                <div class="w-16 h-16 overflow-hidden rounded-full border-2 transition-all duration-300 group-hover:scale-110"
                ...
                <div class="absolute -bottom-1 -right-1 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2"
                     :class="isDark ? 'bg-amber-600 text-white border-gray-800' : 'bg-amber-400 text-amber-900 border-white'">
                  作者
                </div>
              </div>
              <div class="ml-5 flex-1">
                <div class="font-medium text-lg transition-colors duration-300"
                ...
                <div class="flex justify-between items-center mt-2">
                  <a :href="contributors[0].html_url" target="_blank" rel="noopener noreferrer"
                     class="text-base transition-colors duration-200 flex items-center hover:underline"
            ...
            <div v-for="(contributor, index) in contributors.slice(1, 3)" :key="contributor.id"
                 class="flex items-center p-5 rounded-xl transition-all duration-200 group"
                 :class="isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'">
              <div class="relative">
                <div class="w-14 h-14 overflow-hidden rounded-full border-2 transition-all duration-300 group-hover:scale-110"
              ...
              <div class="ml-4 flex-1">
                ...
                <div class="flex justify-between items-center mt-2">
                  <a :href="contributor.html_url" target="_blank" rel="noopener noreferrer"
                     class="text-base transition-colors duration-200 hover:underline"
                     :class="isDark ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'">
                    @{{ contributor.login }}
                  </a>
                  <span class="text-sm px-3 py-1 rounded-full"
        ...
        <div v-if="contributors.length > 3">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg uppercase tracking-wider opacity-70"
          ...
          <!-- 贡献者头像网格 - 使用更大的网格 -->
          <div class="grid grid-cols-6 md:grid-cols-10 lg:grid-cols-15 gap-3">
            <div v-for="contributor in contributors.slice(3, showAllContributors ? undefined : 20)"
                 :key="contributor.id"
                 class="group relative">
              <a :href="contributor.html_url" target="_blank" rel="noopener noreferrer"
                 class="block w-full pb-[100%] rounded-xl overflow-hidden transition-all duration-200 transform hover:scale-110 hover:shadow-lg"
                 :title="`${contributor.login} (${contributor.contributions}次提交)`">
                <img :src="contributor.avatar_url" :alt="contributor.login"
                     class="absolute top-0 left-0 w-full h-full object-cover" />
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end justify-center p-2">
                  <span class="text-[11px] text-white truncate w-full text-center">{{ contributor.login }}</span>
                </div>
              </a>
            </div>
          </div>
          ...
          <!-- 显示更多按钮 -->
          <button v-if="contributors.length > 20 && !showAllContributors"
                  @click="showAllContributors = true"
                  class="w-full mt-6 py-3 text-base rounded-xl transition-colors duration-200"
                  :class="isDark ? 'bg-gray-700 hover:bg-gray-600 text-gray-300' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'">
            显示全部 {{ contributors.length - 20 }} 位贡献者
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-xl border shadow-xl overflow-hidden mb-16 transition-all duration-300"
      ...
      <!-- 加载状态 -->
      <div v-if="loadingLicense" class="p-10 flex flex-col items-center justify-center">
        <div class="w-10 h-10 border-4 border-t-transparent border-blue-500 rounded-full animate-spin mb-6"></div>
        <p class="text-lg" :class="isDark ? 'text-gray-400' : 'text-gray-500'">加载许可证信息...</p>
      </div>

      <!-- 许可证内容 -->
      <div v-else class="p-8">
        <div class="p-6 rounded-xl border border-dashed flex items-start"
             :class="isDark ? 'border-gray-700 bg-gray-750' : 'border-gray-300 bg-gray-50'">
          <Icon icon="mdi:scale-balance" class="text-3xl mr-5 flex-shrink-0 mt-0.5"
                :class="isDark ? 'text-purple-400' : 'text-purple-600'" />
          <div>
            <h3 class="font-medium text-xl transition-colors duration-300"
            ...
            <a href="https://github.com/shadow1ng/fscan/blob/master/LICENSE.txt" target="_blank" rel="noopener noreferrer"
               class="inline-flex items-center mt-5 text-base font-medium hover:underline"
               :class="isDark ? 'text-blue-400' : 'text-blue-600'">
    ...
    <div class="rounded-xl border shadow-xl overflow-hidden mb-16 transition-all duration-300"
      ...
      <div class="p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex items-center p-6 rounded-xl transition-all duration-200 hover:bg-opacity-80"
               :class="isDark ? 'bg-gray-750 hover:bg-gray-700' : 'bg-gray-50 hover:bg-gray-100'">
            <div class="p-3 rounded-xl mr-5"
            ...
            <div>
              ...
              <p class="text-base mt-2" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
                报告问题或提交改进建议
              </p>
            </div>
          </div>
    ...
    <div class="rounded-xl border shadow-xl overflow-hidden mb-16 transition-all duration-300"
      ...
      <!-- 加载状态 -->
      <div v-if="loadingReleases" class="p-10 flex flex-col items-center justify-center">
        <div class="w-12 h-12 border-4 border-t-transparent border-blue-500 rounded-full animate-spin mb-6"></div>
        <p class="text-lg" :class="isDark ? 'text-gray-400' : 'text-gray-500'">加载版本历史...</p>
      </div>

      <!-- 错误提示 -->
      <div v-else-if="releasesError" class="p-10 text-center">
        <div class="p-4 rounded-full bg-red-100 text-red-500 inline-flex items-center justify-center mb-6">
          <Icon icon="mdi:alert-circle" class="text-4xl" />
        </div>
        <p class="text-xl text-red-500 mb-4">无法加载版本历史</p>
        <p class="text-base" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{ releasesError }}</p>
        <button @click="fetchReleases" class="mt-6 px-6 py-3 bg-blue-500 text-white rounded-xl text-base">
          重试
        </button>
      </div>
      ...
      <div v-else class="p-8">
        <div class="space-y-8">
          <div v-for="(version, index) in releases" :key="version.id"
               class="relative pl-10 border-l-2 pb-8 last:pb-0 group"
               :class="index === 0 ?
            ...
            <!-- 版本点 -->
            <div class="absolute -left-3 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 group-hover:scale-110"
                 :class="index === 0 ?
            ...
            <!-- 版本内容 -->
            <div class="ml-2">
              <h3 class="text-xl font-medium flex items-center flex-wrap gap-x-4 gap-y-2"
                  :class="isDark ? 'text-white' : 'text-gray-800'">
                ...
                <span v-if="index === 0" class="px-3 py-1 text-sm font-semibold rounded-full bg-green-500/20 text-green-400">最新版本</span>
              </h3>

              <!-- GitHub API 返回的发布说明是 Markdown 格式 -->
              <div class="mt-5 space-y-3">
                <div v-if="version.body" class="prose prose-lg max-w-none"
                     :class="isDark ? 'prose-invert' : ''">
                  <div v-html="formatReleaseNotes(version.body)"></div>
                </div>
                <div v-else class="text-base italic"
              ...
              <div class="mt-5">
                <a :href="version.html_url" target="_blank" rel="noopener noreferrer"
                   class="inline-flex items-center text-base font-medium hover:underline mr-5"
                   :class="isDark ? 'text-blue-400' : 'text-blue-600'">
                  <Icon icon="mdi:github" class="mr-2" />
                  查看详情
                </a>
                <a :href="version.zipball_url" class="inline-flex items-center text-base font-medium hover:underline"
