<template>
  <div class="mb-16 rounded-xl border overflow-hidden shadow-lg transition-colors duration-300"
    ...
    <div class="px-8 py-6 border-b flex items-center"
      ...
      <Icon icon="mdi:web" class="mr-3 text-2xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      <h2 class="font-medium text-xl" :class="isDark ? 'text-gray-200' : 'text-gray-700'">Web扫描参数</h2>
    </div>
    <div class="p-8">
      <!-- 目标URL和URLs文件 -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <input type="text" v-model="params.u" placeholder="指定单个目标URL"
        ...
        <div>
          ...
          <input type="text" v-model="params.uf" placeholder="URLs列表文件路径"
                 class="w-full px-4 py-3 text-base rounded-lg border outline-none transition-colors duration-200"
                 :class="isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-700 focus:border-blue-500'" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <input type="text" v-model="params.cookie" placeholder="自定义Cookie"
                 class="w-full px-4 py-3 text-base rounded-lg border outline-none transition-colors duration-200"
                 :class="isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-700 focus:border-blue-500'" />
        </div>
        ...
        <div>
          ...
          <input type="range" v-model.number="params.wt" min="1" max="30" step="1"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" v-model="params.proxy" placeholder="HTTP代理地址 (例如: http://127.0.0.1:8080)"
                 class="w-full px-4 py-3 text-base rounded-lg border outline-none transition-colors duration-200"
                 :class="isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-700 focus:border-blue-500'" />
        </div>
        ...
        <div>
          ...
          <input type="text" v-model="params.socks5" placeholder="Socks5代理地址 (例如: 127.0.0.1:1080)"
                 class="w-full px-4 py-3 text-base rounded-lg border outline-none transition-colors duration-200"
                 :class="isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-700 focus:border-blue-500'" />
        </div>
      </div>
