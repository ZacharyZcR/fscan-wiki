    ...
    <div class="rounded-xl border overflow-hidden shadow-lg transition-colors duration-300 mb-16"
        ...
        <div class="px-6 py-4 border-b flex items-center gap-4"
            ...
            <div class="flex items-center shrink-0">
                <Icon icon="mdi:console" class="mr-2 text-xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
                <span class="font-medium text-lg" :class="isDark ? 'text-gray-200' : 'text-gray-700'">扫描结果</span>
            </div>
            <div class="flex-1 px-4">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center">
                        <span class="font-medium text-sm"
                            :class="isDark ? 'text-gray-300' : 'text-gray-600'">扫描状态:</span>
                    ...
                    <span v-if="scanStatus === 'active'" class="text-xs" :class="isDark ? 'text-gray-400' : 'text-gray-500'">
                        进度: {{ scanProgress }}%
                    </span>
                </div>
                ...
                <div class="p-4">
                    <div class="tabs">
                        <div class="tab-list flex space-x-2 border-b mb-4">
                            ...
                            <button @click="setView('json')" :class="[
                                'py-2 px-4 font-medium transition-colors duration-200',
                                view === 'json'
                                    ? isDark
                                        ? 'bg-blue-600 text-white rounded-t-md'
                        ...
                        <div v-if="view === 'table'" class="overflow-x-auto max-h-96" :class="isDark ? 'bg-gray-800' : 'bg-white'">
                                ...
                                <thead :class="isDark ? 'bg-gray-700' : 'bg-gray-50'">
                                    <tr>
                                        <th scope="col"
                                            ...
                                            :class="isDark ? 'text-gray-300' : 'text-gray-500'">Time</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
                                            :class="isDark ? 'text-gray-300' : 'text-gray-500'">Type</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
                                            :class="isDark ? 'text-gray-300' : 'text-gray-500'">Target</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
                                            :class="isDark ? 'text-gray-300' : 'text-gray-500'">Status</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider"
                                            :class="isDark ? 'text-gray-300' : 'text-gray-500'">Details</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200"
                                    :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
                                    <tr v-for="(item, index) in filteredData" :key="index" :class="[
                                        isDark
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span
                                                ...
                                                :class="{
                                                    ...
                                                    'bg-gray-100 text-gray-800': !['PORT', 'SERVICE', 'VULNERABILITY', 'ATTACK'].includes(item.type)
                                                }">
                                                {{ item.type }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{ item.target }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                                                :class="{
                                                    ...
                                                    'bg-gray-100 text-gray-800': !['open', 'identified', 'warning', 'critical'].includes(item.status)
                                                }">
                                                {{ item.status }}
                                            </span>
                                        </td>
                        ...
                        <div v-if="view === 'json'" class="overflow-x-auto max-h-96" :class="isDark ? 'bg-gray-800' : 'bg-white'" >
