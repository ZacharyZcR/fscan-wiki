<template>
  <div class="transition-colors duration-500" :class="isDark ? 'dark text-gray-200' : 'text-gray-800'">
    <!-- 英雄区域 - 视差滚动效果 -->
    <section class="relative overflow-hidden rounded-xl mb-12">
      <!-- 动态背景效果 -->
      <div class="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-700 dark:from-blue-700 dark:to-indigo-900 transform transition-transform duration-10000 animate-gradient-slow">
      ...
      <div class="relative container mx-auto px-4 py-16 md:py-24">
        <div class="max-w-3xl mx-auto text-center transform transition-all duration-700 hover:translate-y-[-5px] hover:scale-[1.02]">
          <div class="inline-flex items-center px-3 py-1 rounded-full bg-blue-800/30 backdrop-blur-sm text-blue-100 text-sm font-medium mb-6 animate-float">
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <router-link to="/documents" class="btn-primary transform transition-all hover:scale-105 hover:rotate-1">
              <Icon icon="mdi:book-open-page-variant" class="mr-2" />
              浏览文档
            </router-link>

            <a href="https://github.com/shadow1ng/fscan/releases/tag/2.0.0-build4"
               target="_blank"
          ...
          <div class="mt-8 flex flex-wrap justify-center gap-6">
            <div class="flex items-center text-blue-100 transition-all duration-300 hover:text-white hover:transform hover:translate-y-[-3px]">
              <Icon icon="mdi:github" class="mr-2" />
              <a href="https://github.com/shadow1ng/fscan" target="_blank"
    ...
    <section class="container mx-auto px-4 mb-16 relative">
      <div class="absolute -left-40 -top-20 w-64 h-64 bg-blue-500 rounded-full filter blur-[150px] opacity-20 animate-blob"></div>
      <div class="absolute -right-40 -bottom-20 w-64 h-64 bg-purple-500 rounded-full filter blur-[150px] opacity-20 animate-blob animation-delay-2000"></div>

      <h2 class="text-2xl md:text-3xl font-bold mb-8 transition-colors duration-300 relative after:content-[''] after:absolute after:bottom-[-6px] after:left-0 after:w-24 after:h-1 after:bg-indigo-500 after:rounded-full"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div v-for="(feature, index) in coreFeatures" :key="index"
          ...
          <div class="w-12 h-12 flex items-center justify-center rounded-lg shrink-0 transition-all duration-500 transform hover:scale-110 hover:rotate-6"
    ...
    <section class="container mx-auto px-4 mb-16">
      <div class="flex justify-between items-center mb-8">
        <h2 class="text-2xl md:text-3xl font-bold transition-colors duration-300 relative after:content-[''] after:absolute after:bottom-[-6px] after:left-0 after:w-24 after:h-1 after:bg-red-500 after:rounded-full"
            :class="isDark ? 'text-white' : 'text-gray-900'">
          GitHub Issues
        </h2>

        <a href="https://github.com/shadow1ng/fscan/issues"
           target="_blank"
          ...
          所有问题
          <Icon icon="mdi:arrow-right" class="ml-1 transition-transform duration-300 group-hover:translate-x-1" />
        </a>
      </div>

      <div class="rounded-xl overflow-hidden transition-colors duration-300 shadow-lg transform hover:shadow-2xl"
        ...
        <div v-else class="divide-y transition-colors duration-300"
             :class="isDark ? 'divide-gray-700' : 'divide-gray-100'">
          <div v-for="(issue, index) in githubIssues" :key="index"
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
              <h3 class="font-semibold transition-colors duration-300 flex items-center"
                  :class="isDark ? 'text-white' : 'text-gray-900'">
                <span class="mr-2 px-2 py-0.5 text-xs rounded-full"
                      :class="issue.state === 'open' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300'">
            ...
            <p class="transition-colors duration-300 line-clamp-2"
        ...
        <div class="p-6 flex items-center justify-center border-t transition-colors duration-300"
             :class="isDark ? 'border-gray-700 bg-gray-750' : 'border-gray-100 bg-gray-50'">
          <a href="https://github.com/shadow1ng/fscan/issues/new"
             target="_blank"
    ...
    <!-- 项目资源 - 卡片翻转效果 -->
    <section class="container mx-auto px-4 mb-16">
      <h2 class="text-2xl md:text-3xl font-bold mb-8 transition-colors duration-300 relative after:content-[''] after:absolute after:bottom-[-6px] after:left-0 after:w-24 after:h-1 after:bg-green-500 after:rounded-full"
          :class="isDark ? 'text-white' : 'text-gray-900'">
        项目资源
      </h2>

      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <a v-for="(resource, index) in projectResources" :key="index"
           :href="resource.link"
           target="_blank"
           class="flex items-center p-5 rounded-xl transition-all duration-500 group transform hover:scale-[1.03] hover:-rotate-1"
           :class="isDark ? 'bg-gray-800 hover:bg-gray-750' : 'bg-white hover:bg-blue-50 shadow-md hover:shadow-xl'">
          <div class="w-10 h-10 flex items-center justify-center rounded-lg shrink-0 mr-4 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6"
    ...
    <!-- CTA 区域 - 背景动画效果 -->
    <section class="container mx-auto px-4 mb-16">
      <div class="rounded-xl overflow-hidden relative transform transition-all duration-500 hover:scale-[1.02]">
        <div class="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-700 dark:from-blue-700 dark:to-indigo-900 animate-gradient">
          <div class="absolute inset-0 bg-[url('/pattern.svg')] opacity-10 animate-pulse-slow"></div>
        </div>

        <div class="relative p-8 md:p-12">
          <div class="max-w-2xl mx-auto text-center">
            <h2 class="text-2xl md:text-3xl font-bold mb-4 text-white text-shadow-subtle animate-float-subtle">
              准备好开始使用 Fscan 了吗？
            </h2>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <router-link to="/documents" class="btn-outline transform transition-all hover:scale-105 hover:rotate-1">
                <Icon icon="mdi:book-open-page-variant" class="mr-2" />
                阅读文档
              </router-link>
            </div>
...
const fetchGithubIssues = async () => {
  ...
  try {
    const response = await fetch('https://api.github.com/repos/shadow1ng/fscan/issues?per_page=5&state=all');
    if (response.ok) {
      const data = await response.json();
