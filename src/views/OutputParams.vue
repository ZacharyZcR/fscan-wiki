<template>
  <div class="mb-16 rounded-xl border overflow-hidden shadow-lg transition-colors duration-300"
    ...
    <div class="px-8 py-6 border-b flex items-center"
      ...
      <Icon icon="mdi:file-document-outline" class="mr-3 text-2xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      <h2 class="font-medium text-xl" :class="isDark ? 'text-gray-200' : 'text-gray-700'">输出与显示控制参数</h2>
    </div>
    <div class="p-8">
      <!-- 输出文件和格式 -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <div class="flex items-center">
            ...
            <input type="text" v-model="params.o" placeholder="结果输出文件名 (默认: result.txt)"
                   class="flex-1 px-4 py-3 text-base border rounded-r-lg outline-none transition-colors duration-200"
                   :class="[
                     outputOptions.no.enabled ?
                       (isDark ? 'bg-gray-700/50 border-gray-600 text-gray-400' : 'bg-gray-100 border-gray-300 text-gray-400') :
                       (isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-700 focus:border-blue-500')
                   ]"
                   :disabled="outputOptions.no.enabled" />
          </div>
          <div v-if="outputOptions.no.enabled" class="mt-2 text-xs"
          ...
          <button v-for="level in logLevels" :key="level.value"
                  ...
                  :class="[
                    params.log === level.value ?
                      (isDark ? 'bg-blue-900/30 border-blue-500/40 text-blue-300' : 'bg-blue-50 border-blue-200 text-blue-700') :
                      (isDark ? 'bg-gray-750 border-gray-700 text-gray-400 hover:bg-gray-700' : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100')
                  ]">
            <Icon :icon="level.icon" class="text-xl mb-1"
                  :class="params.log === level.value ?
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="(option, key) in outputOptions" :key="key"
