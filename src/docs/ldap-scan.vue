<template>
  <div>
    ...
    >
      <p class="mb-4">
        LDAP 扫描是 Fscan
        提供的一个重要功能模块，用于发现网络中的轻量级目录访问协议（LDAP）服务，检测匿名访问漏洞以及进行凭据爆破测试。本指南详细介绍
        LDAP 扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        LDAP（轻量级目录访问协议）是一种用于访问和维护分布式目录信息服务的网络协议，广泛应用于企业身份认证和用户目录管理。Fscan
        的 LDAP 扫描模块可以：
      </p>
      ...
      <p class="mb-4">
        LDAP 认证基于"绑定"（Bind）操作，用于验证连接凭据。Fscan 的 LDAP
        扫描模块支持两种主要的绑定方式：
      </p>
      ...
      <p class="mb-4">
        匿名绑定是指不提供任何凭据的 LDAP 连接方式。如果 LDAP
        服务器允许匿名绑定，可能会导致未授权访问目录信息：
      </p>
...
if anonymousResult.Success {
    ...
    return nil
      ...
      <p class="mb-4">
        在 LDAP 连接函数中，匿名绑定通过 <code>UnauthenticatedBind</code> 方法实现：
      </p>
...
if user != "" {
    ...
    err = l.UnauthenticatedBind("")
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              为提高安全性，建议在生产环境中禁用 LDAP 匿名绑定。这可以通过修改 LDAP
              服务器配置并要求所有连接提供有效凭据来实现。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        认证绑定需要提供有效的用户名（DN）和密码。Fscan 会尝试常见的 DN 格式和用户名密码组合：
      </p>
...
for _, user := range Common.Userdict["ldap"] {
    for _, pass := range Common.Passwords {
      ...
      <p class="mb-4">
        在扫描代码中，使用通用的 DN 格式尝试绑定操作，这能够覆盖许多常见的 LDAP 配置：
      </p>
...
func tryLDAPCredential(ctx context.Context, info *Common.HostInfo, credential LDAPCredential,
    ...
    return &LDAPScanResult{
      ...
      <p class="mb-4">
        LDAP 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 LDAP
          连接尝试的最大时间（秒）
        </li>
        <li>
...
conn, err := dialer.DialContext(ctx, "tcp", address)</code></pre>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              在扫描大型网络时，合理设置超时参数非常重要。LDAP
              服务在高负载下响应可能较慢，建议适当增加超时时间。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        Fscan 的 LDAP 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：
      </p>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              LDAP 目录的搜索基础 DN 通常因企业而异。在实际环境中，您可能需要根据目标环境调整 DN
              格式，例如使用 "uid={user},ou=people,dc=example,dc=com" 等格式。
            </p>
          </div>
        </div>
      </div>
...
const ldapParameters = [
  ...
  {
    name: '-p 389,636',
    description: 'LDAP 常用端口，389 是标准端口，636 是 LDAPS 加密端口',
    example: '-p 389,636',
  },
