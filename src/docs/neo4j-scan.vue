<template>
  <div>
    ...
    >
      <p class="mb-4">
        Neo4j 扫描是 Fscan 的一个专业功能模块，用于发现网络中运行的 Neo4j
        图数据库服务，并检测无认证访问、默认凭据和弱密码等安全问题。本指南详细介绍 Neo4j
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        Neo4j 是领先的图数据库平台，被广泛应用于数据关系分析、推荐系统、网络和 IT 运维等领域。Fscan
        的 Neo4j 扫描模块可以：
      </p>
      ...
      <div
        ...
        :class="
          isDark ? 'bg-cyan-900/10 border border-cyan-900/20' : 'bg-cyan-50 border border-cyan-100'
        "
      >
        <div class="flex">
          <Icon
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div class="flex items-center mb-2">
            ...
            >
              <span class="text-xs font-bold" :class="isDark ? 'text-cyan-400' : 'text-cyan-600'"
                >1</span
              >
            </div>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in neo4jParameters"
              :key="param.name"
              :class="isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-50'"
            >
              <td class="py-2 px-4 font-mono" :class="isDark ? 'text-cyan-400' : 'text-cyan-600'">
                {{ param.name }}
              </td>
...
for _, credential := range initialCredentials {
    ...
    if result.Success {
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Neo4j 的默认安装通常需要认证，但配置不当可能导致无认证访问。默认用户名和密码都是
              "neo4j"，首次登录后会要求修改密码，但很多测试或开发环境可能没有变更默认凭据。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        Neo4j 扫描使用 Bolt 协议进行连接和验证。Bolt 是 Neo4j 的二进制协议，提供更高效的数据库通信：
      </p>
...
func saveNeo4jResult(info *Common.HostInfo, target string, result *Neo4jScanResult) {
    ...
    Common.SaveResult(vulnResult)
      ...
      <p class="mb-4">
        Neo4j 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
class="transition-colors duration-300"
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 Neo4j
          连接尝试的最大时间（秒）
        </li>
        <li>
...
config := func(c *neo4j.Config) {
    ...
    c.ConnectionAcquisitionTimeout = timeout
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Neo4j
              数据库在处理复杂查询时可能需要较长时间，建议根据网络环境和数据库规模设置合理的超时时间，避免因超时过短而导致漏报。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        Fscan 的 Neo4j 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：
      </p>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-cyan-200' : 'text-cyan-800'">
              Neo4j
              通常用于存储和处理敏感数据关系，应确保其安全配置。建议始终启用认证、使用强密码、限制网络访问，并定期更新到最新版本以获取安全补丁。
            </p>
          </div>
        </div>
      </div>
