<template>
  <div>
    ...
    >
      <p class="mb-4">
        FTP 扫描是 Fscan
        的重要功能模块，用于发现网络中运行的文件传输协议（FTP）服务器，检测匿名访问漏洞以及进行密码爆破。本指南详细介绍
        FTP 扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        ...
        Protocol，FTP）是一种用于在计算机网络上进行文件传输的标准网络协议。Fscan 的 FTP
        扫描模块可以：
      </p>
      ...
      <p class="mb-4">
        匿名访问是 FTP 协议中的一项特性，允许用户无需提供有效的用户名和密码即可登录 FTP
        服务器。Fscan 首先会尝试使用匿名凭据进行登录：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              匿名 FTP 访问在生产环境中通常是一个严重的安全风险。在没有必要的情况下，建议禁用 FTP
              服务器的匿名访问功能，或者将匿名用户限制在只读目录中。
            </p>
          </div>
        </div>
      </div>
...
func concurrentFtpScan(ctx context.Context, info *Common.HostInfo, credentials []FtpCredential,
    ...
    // ...省略剩余实现...
      ...
      <p class="mb-4">
        FTP 连接使用第三方库 <code>github.com/jlaffaye/ftp</code> 实现，支持登录认证和目录列表获取：
      </p>
...
func FtpConn(info *Common.HostInfo, user string, pass string) (success bool, directories []string, err error) {
    ...
    return true, directories, nil
      ...
      <p class="mb-4">
        FTP 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 FTP
          连接尝试的最大时间（秒）
        </li>
        <li>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              FTP 服务器通常会限制同时连接的数量。如果在扫描过程中遇到"too many
              connections"错误，程序会自动等待 5 秒后重试，以避免过度消耗服务器资源。
            </p>
          </div>
        </div>
      </div>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        这个配置使用 50 个线程，单个连接超时设置为 3 秒，最大重试次数为 2，并将结果保存到文件。
      </p>
...
class="transition-colors duration-300"
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        ...
        <li>
          <strong>启用加密</strong>：考虑使用 FTPS（FTP over SSL/TLS）或 SFTP（SSH File Transfer
          Protocol）代替明文 FTP
        </li>
      ...
      <p class="mb-4">
        通过实施这些安全措施，可以显著提高 FTP 服务器的安全性，减少被扫描和攻击的风险。
      </p>
    </div>
