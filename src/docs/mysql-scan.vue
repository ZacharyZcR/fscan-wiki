<template>
  <div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">MySQL 扫描是 Fscan 的核心功能之一，用于发现网络中的 MySQL 数据库服务并尝试进行弱密码检测。本指南详细介绍 MySQL 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">MySQL 是全球最流行的开源关系型数据库管理系统之一，广泛应用于各类 Web 应用和企业信息系统。Fscan 的 MySQL 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              MySQL 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">凭据准备</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <p class="mb-4">扫描实现采用高效的工作池和上下文管理，确保即使在大规模扫描时也能保持稳定性和性能。扫描过程中会考虑全局超时和单个连接超时，避免资源浪费。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in mysqlParameters" :key="param.name"
      ...
      <p class="mb-4">MySQL 扫描模块使用官方的 Go MySQL 驱动进行连接验证，这确保了与各种 MySQL 版本的良好兼容性。</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              当凭据错误时，MySQL 服务器返回包含 "Access denied" 的错误消息。扫描器会识别这种错误，并避免对明确错误的凭据进行重试。
            </p>
          </div>
        </div>
      </div>
...
func tryMySQLCredential(ctx context.Context, info *Common.HostInfo, credential MySQLCredential,
    ...
    return &MySQLScanResult{Success: false, Error: lastErr, Credential: credential}
      ...
      <p class="mb-4">MySQL 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 MySQL 连接尝试的最大时间（秒）</li>
...
connStr := fmt.Sprintf(
    ...
    username, password, host, port, timeout,
      ...
      <p class="mb-4">Fscan 的 MySQL 扫描模块实现了智能重试机制，可通过 <code>-retries</code> 参数设置最大重试次数：</p>
      ...
      <p class="mb-4">重试机制会自动跳过不需要重试的错误（如 Access denied），只对可能是临时网络故障的错误进行重试：</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              在MySQL扫描中，常见的默认用户有 root、mysql、admin 等。针对目标环境定制用户名和密码字典可以显著提高扫描效率。使用 {user} 变量可以根据用户名自动生成常见密码模式。
            </p>
          </div>
        </div>
      </div>
