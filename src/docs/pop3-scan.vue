<template>
  <div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">POP3 扫描是 Fscan 的一个专用功能模块，用于发现网络中运行的 POP3 邮件服务器，并测试邮箱账户的弱密码。本指南详细介绍 POP3 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">POP3（Post Office Protocol 3）是一种用于从远程服务器获取电子邮件的标准协议。Fscan 的 POP3 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              POP3 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对邮件服务器进行扫描不仅违反法律法规，还可能导致封禁 IP 或触发安全警报。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">POP3 扫描模块采用特殊的低速扫描策略，减少对目标服务器的冲击，并避免触发安全限制：</p>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">连接尝试</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">低速认证</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">智能错误处理</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <p class="mb-4">POP3 扫描实现与其他服务扫描模块的主要差异在于其特殊的低速和单线程设计，这是因为邮件服务器通常对连接数和认证尝试次数有严格限制。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in pop3Parameters" :key="param.name"
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">特别注意</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              尽管提供了 <code>-t</code> 参数，POP3 扫描默认会强制使用单线程（maxConcurrent = 1）进行扫描，以避免触发邮件服务器的防护机制。
            </p>
          </div>
        </div>
      </div>
...
func tryPOP3Auth(conn net.Conn, user string, pass string, timeout time.Duration) (bool, error) {
    ...
    return false, fmt.Errorf("认证失败: %s", strings.TrimSpace(response))
      ...
      <ol class="pl-5 list-decimal space-y-2 mb-6">
        ...
        <li><strong>接收 PASS 命令响应</strong>：服务器返回 <code>+OK</code> 表示密码正确，认证成功</li>
      </ol>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">POP3 响应格式</p>
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              POP3 协议中所有成功的响应都以 <code>+OK</code> 开头，表示命令执行成功；错误响应通常以 <code>-ERR</code> 开头，
              后面跟随错误详情。通过检查响应内容可以确定认证结果。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">POP3 服务通常同时支持明文连接和 TLS 加密连接。Fscan 会按顺序尝试这两种连接方式：</p>
    ...
    case <-rateLimiter:
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          <Icon icon="mdi:lightbulb" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-amber-400' : 'text-amber-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">为什么需要限速？</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              邮件服务器通常设有严格的安全措施，包括：1) 连接频率限制；2) 认证失败次数限制；3) IP 封禁机制。
              过快的扫描不仅会失败，还可能导致 IP 被服务器临时或永久封禁。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">POP3 扫描模块实现了智能的错误处理和重试策略，特别关注邮件服务器常见的限制错误：</p>
...
func tryPOP3Credential(ctx context.Context, info *Common.HostInfo, credential POP3Credential,
    ...
    // ...
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        ...
        <li><strong>自适应延迟</strong>：遇到连接限制时，使用更长的等待时间（(retry+1) * 3000ms）</li>
      ...
      <p class="mb-4">这个配置使用更长的单个连接超时（10秒）和更多的重试次数（5次），并将结果保存到文件。</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              针对 POP3 服务的扫描应特别注意以下几点：1) 使用较少的用户名和密码组合，避免触发账户锁定；
              2) 扫描时间可能较长，耐心等待；3) 避免在短时间内重复扫描同一目标。
            </p>
          </div>
        </div>
      </div>
...
const pop3Parameters = [
  ...
  { name: '-p 110,995', description: '指定 POP3 端口，110 为普通连接，995 为 SSL/TLS 加密连接', example: '-p 110,995' },
  { name: '-user', description: '指定用户名列表，多个用逗号分隔', example: '-user user1,admin' },
  { name: '-pwd', description: '指定密码列表，多个用逗号分隔，支持 {user} 变量', example: '-pwd 123456,{user}@2023' },
