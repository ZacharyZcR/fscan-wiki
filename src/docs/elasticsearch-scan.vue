  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:database-search" class="text-xl" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'" />
      </div>
    ...
    <div class="mb-6 rounded-lg p-3 transition-colors duration-300"
      ...
      <ul class="pl-4 space-y-1">
        <li v-for="(section, index) in contentSections" :key="index"
            ...
            :class="isDark ? 'text-gray-400 hover:text-yellow-400' : 'text-gray-600 hover:text-yellow-600'">
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">Elasticsearch 扫描是 Fscan 的核心功能之一，用于发现网络中的 Elasticsearch 服务并检测可能的未授权访问和弱口令漏洞。本指南详细介绍 Elasticsearch 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Elasticsearch 是一个分布式、RESTful 风格的搜索和分析引擎，广泛应用于全文搜索、日志分析和实时数据分析。Fscan 的 Elasticsearch 扫描模块可以：</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
           :class="isDark ? 'bg-yellow-900/10 border border-yellow-900/20' : 'bg-yellow-50 border border-yellow-100'">
        <div class="flex">
          <Icon icon="mdi:shield-alert" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-yellow-300' : 'text-yellow-700'">安全提示</p>
            <p class="mt-1" :class="isDark ? 'text-yellow-200' : 'text-yellow-800'">
              Elasticsearch 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未授权访问的 Elasticsearch 服务可能包含敏感信息，未经许可的访问可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
              ...
              <span class="text-xs font-bold" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'">1</span>
            </div>
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-yellow-900/20' : 'bg-yellow-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'">2</span>
            </div>
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-yellow-900/20' : 'bg-yellow-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'">3</span>
            </div>
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">身份验证爆破</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-yellow-900/20' : 'bg-yellow-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'">4</span>
            </div>
      ...
      <p class="mb-4">Elasticsearch 扫描实现基于 HTTP 请求而非 TCP 连接，使用 Elasticsearch 的 REST API 进行验证。扫描过程中会考虑全局超时和单个连接超时，避免资源浪费。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in elasticParameters" :key="param.name"
      ...
      <p class="mb-4">Elasticsearch 支持多种认证方式，Fscan 主要使用 HTTP Basic 认证进行身份验证：</p>
...
func ElasticConn(ctx context.Context, info *Common.HostInfo, user string, pass string, timeoutSeconds int64) (bool, error) {
    ...
    return resp.StatusCode == 200, nil
      ...
      <p class="mb-4">Fscan 的扫描模块检测 <code>_cat/indices</code> API 是否返回 200 状态码来判断身份验证是否成功。</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Elasticsearch 6.0 及更高版本默认启用了 X-Pack 安全功能，而早期版本可能未默认启用认证。Fscan 支持检测这两种情况。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">未授权访问是 Elasticsearch 常见的安全问题。Fscan 首先会尝试无认证访问来检测是否存在这一漏洞：</p>
    ...
    Common.SaveResult(result)
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          <Icon icon="mdi:shield-alert" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-yellow-400' : 'text-yellow-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-yellow-300' : 'text-yellow-700'">安全风险</p>
            <p class="mt-1" :class="isDark ? 'text-yellow-200' : 'text-yellow-800'">
              未授权访问的 Elasticsearch 可能导致数据泄露、数据篡改甚至远程代码执行风险。建议立即为 Elasticsearch 启用认证和访问控制。
            </p>
          </div>
        </div>
      </div>
...
func concurrentElasticScan(ctx context.Context, info *Common.HostInfo, credentials []ElasticCredential,
    ...
    // ...省略剩余实现...
      ...
      <p class="mb-4">Elasticsearch 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 HTTP 请求的最大时间（秒）</li>
...
client := &http.Client{
    ...
    Transport: &http.Transport{
      ...
      <p class="mb-4">Fscan 的 Elasticsearch 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              保护 Elasticsearch 的最佳实践包括：启用 X-Pack 安全功能、配置强密码、使用 TLS/SSL 加密、限制网络访问、定期更新版本，以及使用 Elastic Stack 安全功能监控异常访问。
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="mt-8 pt-4 border-t transition-colors duration-300 text-sm"
      ...
      <a href="https://github.com/shadow1ng/fscan/issues" target="_blank"
         ...
         :class="isDark ? 'text-yellow-400 hover:text-yellow-300' : 'text-yellow-600 hover:text-yellow-700'">
...
const elasticParameters = [
  ...
  { name: '-p 9200', description: '指定 Elasticsearch 端口（默认为 9200）', example: '-p 9200,9300' },
