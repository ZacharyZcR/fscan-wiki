<template>
  <div>
    ...
    >
      <p class="mb-4">
        Kafka 扫描是 Fscan 的重要功能模块，用于发现网络中运行的 Apache Kafka
        消息队列服务，并检测未授权访问和弱密码问题。本指南详细介绍 Kafka
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        Apache Kafka 是一个分布式的流处理平台，被广泛用于构建实时数据流管道和流式应用程序。Fscan 的
        Kafka 扫描模块可以：
      </p>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in kafkaParameters"
              :key="param.name"
      ...
      <p class="mb-4">
        Kafka 未授权访问是一个常见的安全隐患。许多组织未正确配置 Kafka
        的身份验证，导致服务可以在没有任何凭据的情况下被访问。Fscan 会首先尝试无认证连接：
      </p>
...
func KafkaConn(info *Common.HostInfo, user string, pass string) (bool, error) {
    ...
    // 尝试连接...
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              防止 Kafka 未授权访问的关键措施包括启用 SASL 认证、配置 ACL 访问控制、使用 SSL/TLS
              加密，以及使用防火墙限制对 Kafka 端口的访问。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        当 Kafka 服务启用了 SASL 认证，Fscan 会自动尝试使用内置用户名和密码字典进行爆破：
      </p>
...
func concurrentKafkaScan(ctx context.Context, info *Common.HostInfo, credentials []KafkaCredential,
    ...
    // ...省略剩余实现...
      ...
      <p class="mb-4">
        Kafka 扫描模块使用 Sarama
        库实现连接与验证功能。它同时尝试两种连接方式：作为消费者和作为客户端连接，这增加了检测成功率：
      </p>
...
return false, err</code></pre>
      ...
      <p class="mb-4">
        这种双重检测方法提高了验证的可靠性，因为某些 Kafka
        配置可能只允许特定类型的连接。扫描过程中还会分析错误信息，以判断失败原因是否与认证相关。
      </p>
      ...
      <p class="mb-4">
        Kafka 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Kafka 连接在大型集群或网络延迟较高的环境中可能需要更长的超时时间。建议使用
              <code>-time</code> 参数适当调整单个连接的超时时间。
            </p>
          </div>
        </div>
      </div>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        这个命令会扫描使用非标准端口（29092）的 Kafka 服务。在容器化环境或某些云平台中，Kafka
        可能使用非默认端口。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              在扫描 Kafka 服务时，考虑扩展用户名字典，包括常见的 Kafka 相关用户名如
              "kafka"、"admin"、"service"、 "consumer" 等，以及与目标组织相关的特定用户名。
            </p>
          </div>
        </div>
      </div>
