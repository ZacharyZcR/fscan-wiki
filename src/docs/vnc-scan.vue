  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:remote-desktop" class="text-xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      </div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">VNC 扫描是 Fscan 的重要功能之一，用于发现网络中的 VNC（Virtual Network Computing）服务，并通过密码爆破尝试进行远程访问。本指南详细介绍 VNC 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">VNC（Virtual Network Computing）是一种图形桌面共享系统，允许用户远程连接到其他计算机，查看和操作其桌面。Fscan 的 VNC 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              VNC 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">密码列表构建</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">并发认证尝试</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in vncParameters" :key="param.name"
      ...
      <p class="mb-4">VNC 使用基于 VNC 协议的认证机制。Fscan 使用 <code>go-vnc</code> 库实现与 VNC 服务器的通信和认证：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              与其他服务不同，VNC 认证通常只需要密码，不需要提供用户名。这使得 VNC 密码爆破相对简单，但也意味着 VNC 服务器应该使用更强的密码保护。
            </p>
          </div>
        </div>
      </div>
...
func tryVncCredential(ctx context.Context, info *Common.HostInfo, credential VncCredential,
    ...
    return &VncScanResult{Success: false, Error: lastErr, Credential: credential}
      ...
      <p class="mb-4">VNC 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 VNC 连接尝试的最大时间（秒）</li>
...
cancel()</code></pre>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              VNC 连接可能比其他服务需要更长的响应时间，特别是在网络延迟较高的环境。推荐设置稍长的连接超时时间（3-5 秒）以减少误报。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">VNC 扫描支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              VNC 服务器常见的弱密码包括空密码、默认密码（如 "password"、"vnc"）以及简单的数字组合（如 "123456"、"12345678"）。确保在扫描时包含这些常见密码。
            </p>
          </div>
        </div>
      </div>
...
const vncParameters = [
  ...
  { name: '-pwd', description: '指定密码列表，多个用逗号分隔', example: '-pwd 123456,password,vnc' },
