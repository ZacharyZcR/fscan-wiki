  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:database-outline" class="text-xl" :class="isDark ? 'text-orange-400' : 'text-orange-600'" />
      </div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">Oracle 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的 Oracle 数据库服务，并尝试通过弱密码进行身份验证。本指南详细介绍 Oracle 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Oracle 数据库是企业中广泛使用的关系型数据库管理系统，通常包含关键业务数据。Fscan 的 Oracle 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-orange-200' : 'text-orange-800'">
              Oracle 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对数据库系统进行扫描可能违反法律法规，并可能触发数据库的安全监控告警。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">高危凭据检测</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">服务名枚举</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">并发密码爆破</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in oracleParameters" :key="param.name"
      ...
      <p class="mb-4">Oracle 数据库中存在一些常见的默认或高危账户，这些账户往往是攻击者首选的目标。Fscan 首先会尝试这些高危凭据组合：</p>
    ...
    if result != nil && result.Success {
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">高危账户说明</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              SYS 是 Oracle 最高权限账户，具有 SYSDBA 特权；SYSTEM 是主要管理账户。这些账户的默认密码在安装后经常未被修改，构成重大安全风险。
            </p>
          </div>
        </div>
      </div>
    ...
    if result.Success {
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          <Icon icon="mdi:lightbulb" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-amber-400' : 'text-amber-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">SYSDBA 权限说明</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              SYSDBA 是 Oracle 中最高级别的系统权限，拥有此权限可以启动/关闭数据库、创建数据库、完全访问 SYS 用户对象，甚至在数据库关闭状态下执行操作。
            </p>
          </div>
        </div>
      </div>
...
func concurrentOracleScan(ctx context.Context, info *Common.HostInfo, credentials []OracleCredential,
    ...
    // ...省略发送工作和等待结果的代码...
      ...
      <p class="mb-4">Oracle 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 Oracle 连接尝试的最大时间（秒）</li>
...
connStr := fmt.Sprintf("oracle://%s:%s@%s:%s/%s?connect_timeout=%d",
    user, pass, host, port, serviceName, Common.Timeout)</code></pre>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Oracle 数据库连接可能较慢，特别是在远程网络环境中。建议根据网络环境适当增加单个连接超时时间，避免漏报。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">Fscan 的 Oracle 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：</p>
...
func tryOracleCredential(ctx context.Context, info *Common.HostInfo, credential OracleCredential,
    ...
    return &OracleScanResult{Success: false, Error: lastErr, Credential: credential}
      ...
      <h2 id="section-8" class="text-xl font-bold mt-8 mb-4 pb-2 border-b transition-colors duration-300"
          :class="isDark ? 'text-white border-gray-700' : 'text-gray-800 border-gray-200'">
        应用场景与示例
      </h2>
      ...
      <p class="mb-4">这个配置使用 30 个线程，单个连接超时设置为 5 秒，最大重试次数为 2，并将结果保存到文件。</p>

      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
           :class="isDark ? 'bg-orange-900/10 border border-orange-900/20' : 'bg-orange-50 border border-orange-100'">
        <div class="flex">
          <Icon icon="mdi:check-circle" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-orange-400' : 'text-orange-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-orange-300' : 'text-orange-700'">最佳实践</p>
            <p class="mt-1" :class="isDark ? 'text-orange-200' : 'text-orange-800'">
              成功爆破 Oracle 数据库后，建议记录服务名，因为同一服务器上可能有多个 Oracle 实例运行在同一端口上，但服务名不同。
            </p>
          </div>
        </div>
      </div>
