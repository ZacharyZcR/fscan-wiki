  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:database-outline" class="text-xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      </div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">Memcached 扫描是 Fscan 的一项重要功能，用于发现网络中的 Memcached 服务并检测未授权访问漏洞。本指南详细介绍 Memcached 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Memcached 是一个高性能的分布式内存对象缓存系统，常用于缓存数据库查询、API调用结果等，以提高动态Web应用的性能。Fscan 的 Memcached 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              未授权访问的 Memcached 服务可能导致数据泄露、被用于 DDoS 放大攻击或成为内网渗透的入口点。仅在授权的安全测试环境中使用此功能。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">Memcached 扫描模块采用简单有效的检测方法，通过以下步骤识别未授权的 Memcached 服务：</p>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">发送测试命令</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">分析响应</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <p class="mb-4">整个扫描过程采用并发设计，使用上下文（context）处理超时和取消操作，确保即使在大规模扫描时也能高效运行。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in memcachedParameters" :key="param.name"
      ...
      <p class="mb-4">Memcached 未授权访问检测的核心是 <code>tryMemcachedConnection</code> 函数，它通过以下步骤执行检测：</p>
...
Common.LogSuccess(fmt.Sprintf("Memcached %s 未授权访问", realhost))</code></pre>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">Memcached stats 命令</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Memcached 的 stats 命令会返回大量服务器统计信息，包括版本号、内存使用情况、命中率、运行时间等关键指标。这些信息对攻击者分析系统非常有价值。
            </p>
          </div>
        </div>
      </div>
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数控制每个 Memcached 连接尝试的最大时间</li>
        <li><strong>全局超时</strong>：通过 <code>-timeout</code> 参数控制整个扫描任务的最大执行时间</li>
      </ul>
...
defer connCancel()</code></pre>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              在扫描大型网络时，合理设置超时参数非常重要。推荐的连接超时时间为 3-5 秒，全局超时时间视网络规模而定，通常为 300-600 秒。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">虽然 Memcached 扫描本身不支持并发扫描多个目标，但 Fscan 的全局扫描引擎支持多线程扫描，可以通过 <code>-t</code> 参数控制：</p>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-start mb-2">
            <div class="w-6 h-6 rounded-md flex-shrink-0 flex items-center justify-center mr-2 mt-0.5"
                 :class="isDark ? 'bg-green-900/20' : 'bg-green-100'">
              <Icon icon="mdi:check" class="text-sm" :class="isDark ? 'text-green-400' : 'text-green-600'" />
            </div>
            <div>
              <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">绑定内部IP</h3>
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-start mb-2">
            ...
            <div>
              <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">防火墙限制</h3>
              <p class="text-sm mt-1" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-start mb-2">
            ...
            <div>
              <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">SASL 认证</h3>
              <p class="text-sm mt-1" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-start mb-2">
            ...
            <div>
              <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">禁用 UDP</h3>
              <p class="text-sm mt-1" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              定期扫描内部网络中的 Memcached 服务，确保没有暴露未授权访问的实例，是企业安全自查的重要部分。扫描后及时修复发现的问题，并建立长期监控机制。
            </p>
          </div>
        </div>
      </div>
