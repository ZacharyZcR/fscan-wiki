  ...
  <div>
    ...
    >
      <p class="mb-4">
        ...
        服务，并尝试检测未授权访问和弱密码漏洞。本指南详细介绍 Telnet
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        Telnet 是一种应用层协议，基于 TCP 协议，提供远程终端连接服务。虽然 Telnet
        因其明文传输特性在安全性方面存在缺陷，但在许多遗留系统和设备中仍然广泛使用。Fscan 的 Telnet
        扫描模块可以：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Telnet
              扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规。建议使用更安全的
              SSH 替代 Telnet。
            </p>
          </div>
        </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div class="flex items-center mb-2">
            ...
            >
              <span class="text-xs font-bold" :class="isDark ? 'text-amber-400' : 'text-amber-600'"
                >1</span
              >
            </div>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in telnetParameters"
              :key="param.name"
              :class="isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-50'"
            >
              <td class="py-2 px-4 font-mono" :class="isDark ? 'text-amber-400' : 'text-amber-600'">
                {{ param.name }}
              </td>
      ...
      <p class="mb-4">
        Telnet 是一种基于文本的协议，需要处理各种控制字符和命令。Fscan 实现了完整的 Telnet
        协议处理逻辑：
      </p>
...
func (c *TelnetClient) SerializationResponse(responseBuf []byte) (displayBuf []byte, commandList [][]byte) {
    ...
    return displayBuf, commandList
      ...
      <p class="mb-4">
        该函数解析 Telnet 服务器返回的原始字节流，分离出实际显示内容和需要回复的命令列表，是实现正确
        Telnet 交互的关键。
      </p>
      ...
      <p class="mb-4">
        Telnet 扫描模块能够自动识别多种服务器类型，并根据不同类型采用相应的认证策略：
      </p>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              UnauthorizedAccess（无需认证）类型对应的是没有设置密码保护的 Telnet
              服务，这类服务存在严重的安全风险，应立即配置密码保护或关闭服务。
            </p>
          </div>
        </div>
      </div>
...
func (c *TelnetClient) isLoginSucceed(responseString string) bool {
    ...
    return false
      ...
      <p class="mb-4">
        Telnet 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
func concurrentTelnetScan(ctx context.Context, info *Common.HostInfo, credentials []TelnetCredential,
    ...
    // ... 省略后续实现 ...
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 Telnet
          连接尝试的最大时间（秒）
        </li>
        <li>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Telnet
              通常比其他服务需要更长的响应时间，因为它涉及交互式会话和多步骤对话。建议设置更长的超时时间（如
              5-10 秒）以提高扫描准确性。
            </p>
          </div>
        </div>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        这个命令专门针对网络设备（如路由器、交换机）进行 Telnet
        扫描，设置了较长的超时时间以适应网络设备的响应特性。
      </p>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        这个配置使用 50 个线程，单个连接超时设置为 5 秒，最大重试次数为
        2，并将结果保存到文件，适合大规模扫描。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              由于 Telnet 安全性较低，建议使用 SSH 替代所有 Telnet
              服务。对于无法立即替换的遗留系统，应启用强密码保护，限制访问 IP
              范围，并在可能的情况下实施双因素认证。
            </p>
          </div>
        </div>
