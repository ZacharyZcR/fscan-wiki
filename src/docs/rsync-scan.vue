  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:sync" class="text-xl" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'" />
      </div>
    ...
    <div class="mb-6 rounded-lg p-3 transition-colors duration-300"
      ...
      <ul class="pl-4 space-y-1">
        <li v-for="(section, index) in contentSections" :key="index"
            ...
            :class="isDark ? 'text-gray-400 hover:text-indigo-400' : 'text-gray-600 hover:text-indigo-600'">
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">Rsync 扫描是 Fscan 的一个重要功能，用于发现网络中的 Rsync 服务，检测匿名访问漏洞以及进行凭据爆破。本指南详细介绍 Rsync 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Rsync（Remote Synchronization）是一个用于文件传输和同步的开源工具，常用于服务器备份、数据同步和镜像站点维护。Fscan 的 Rsync 扫描模块可以：</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
           :class="isDark ? 'bg-indigo-900/10 border border-indigo-900/20' : 'bg-indigo-50 border border-indigo-100'">
        <div class="flex">
          <Icon icon="mdi:shield-alert" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-indigo-300' : 'text-indigo-700'">安全提示</p>
            <p class="mt-1" :class="isDark ? 'text-indigo-200' : 'text-indigo-800'">
              Rsync 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
              ...
              <span class="text-xs font-bold" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'">1</span>
            </div>
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-indigo-900/20' : 'bg-indigo-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'">2</span>
            </div>
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">模块获取</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-indigo-900/20' : 'bg-indigo-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'">3</span>
            </div>
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            <div class="w-6 h-6 rounded-md flex items-center justify-center mr-2"
                 :class="isDark ? 'bg-indigo-900/20' : 'bg-indigo-100'">
              <span class="text-xs font-bold" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'">4</span>
            </div>
      ...
      <p class="mb-4">Rsync 扫描实现采用高效的协程池和上下文管理，确保即使在大规模扫描时也能保持稳定性和性能。扫描过程中会考虑全局超时和单个连接超时，避免资源浪费。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in rsyncParameters" :key="param.name"
      ...
      <p class="mb-4">Rsync 扫描模块通过与 Rsync 服务器进行一系列协议交互来检测服务状态和尝试认证：</p>
    ...
    continue
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">交互提示</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Rsync 协议的交互是基于文本的，使用 @RSYNCD 前缀进行指令传递。服务器会给出明确响应指示是否需要认证或已成功访问。
            </p>
          </div>
        </div>
      </div>
...
if strings.Contains(authResponse, "@RSYNCD: OK") {
    ...
    if user == "" && pass == "" {
      ...
      <p class="mb-4">当模块响应 "@RSYNCD: OK" 且我们使用空用户名和密码尝试时，表示该模块允许匿名访问。这通常是由于配置不当导致的安全问题。</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          <Icon icon="mdi:lightbulb" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-amber-400' : 'text-amber-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">防护建议</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              防止 Rsync 匿名访问的关键措施是在 rsyncd.conf 配置文件中为每个模块设置 auth users 和 secrets file 选项，确保所有访问都需要身份验证。
            </p>
          </div>
        </div>
      </div>
...
for _, user := range Common.Userdict["rsync"] {
    for _, pass := range Common.Passwords {
      ...
      <p class="mb-4">Rsync 认证格式非常简单，用户名和密码以空格分隔发送。需要注意的是，Rsync 的密码通常存储在服务器的 secrets file 中，格式为 "username:password"。</p>
      ...
      <p class="mb-4">Rsync 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 Rsync 连接尝试的最大时间（秒）</li>
...
dialer := &net.Dialer{
    ...
    default:
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
           :class="isDark ? 'bg-indigo-900/10 border border-indigo-900/20' : 'bg-indigo-50 border border-indigo-100'">
        <div class="flex">
          <Icon icon="mdi:check-circle" class="text-lg flex-shrink-0 mt-0.5 mr-2" :class="isDark ? 'text-indigo-400' : 'text-indigo-600'" />
          <div>
            <p class="font-medium" :class="isDark ? 'text-indigo-300' : 'text-indigo-700'">最佳实践</p>
            <p class="mt-1" :class="isDark ? 'text-indigo-200' : 'text-indigo-800'">
              在执行 Rsync 扫描时，首先推荐仅检测匿名访问问题，这是最常见的 Rsync 安全漏洞。如果需要进行全面的密码爆破，建议限制并发数量，以避免对目标服务器造成过大压力。
            </p>
          </div>
        </div>
      </div>
