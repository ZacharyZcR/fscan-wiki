<template>
  <div>
    ...
    >
      <p class="mb-4">
        ActiveMQ 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的 Apache ActiveMQ
        消息中间件服务，并尝试通过弱密码进行身份验证。本指南详细介绍 ActiveMQ
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        Apache ActiveMQ
        是一个流行的开源消息中间件，广泛应用于企业消息系统、微服务架构和分布式应用。Fscan 的
        ActiveMQ 扫描模块可以：
      </p>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in activemqParameters"
              :key="param.name"
              :class="isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-50'"
            >
              <td class="py-2 px-4 font-mono" :class="isDark ? 'text-green-400' : 'text-green-600'">
                {{ param.name }}
              </td>
      ...
      <p class="mb-4">
        ActiveMQ 扫描模块使用 STOMP 协议来验证凭据。STOMP（Simple Text Oriented Messaging
        Protocol）是一种简单的基于文本的消息传递协议，被 ActiveMQ 等消息中间件广泛支持。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              成功认证时，ActiveMQ 服务器会返回包含 "CONNECTED" 的响应；认证失败时，会返回包含
              "Authentication failed" 或 "ERROR" 的响应。
            </p>
          </div>
        </div>
      </div>
...
func concurrentActiveMQScan(ctx context.Context, info *Common.HostInfo, credentials []ActiveMQCredential,
    ...
    // ...省略剩余实现...
      ...
      <p class="mb-4">
        ActiveMQ 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 ActiveMQ
          连接尝试的最大时间（秒）
        </li>
        <li>
...
conn, err := dialer.DialContext(ctx, "tcp", addr)</code></pre>
      ...
      <p class="mb-4">
        Fscan 的 ActiveMQ 扫描模块支持自动重试连接，通过
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              通常情况下，ActiveMQ
              服务管理员会修改默认的用户名和密码。在扫描中添加目标环境常用的用户名和密码，可以提高扫描成功率。
            </p>
          </div>
        </div>
      </div>
...
const activemqParameters = [
  ...
  {
    name: '-p 61616,8161',
    description: 'ActiveMQ 常用端口，多个用逗号分隔',
    example: '-p 61616,8161',
  },
