<template>
  <div>
    ...
    >
      <p class="mb-4">
        Fscan
        是一款内网综合扫描工具，包含端口扫描、服务识别、网络资产探测、密码爆破等功能。本指南将帮助您快速开始使用
        Fscan。
      </p>
      ...
      <p class="mb-4">
        Fscan 是一个跨平台的工具，支持 Windows、Linux 和 MacOS 系统。您可以从 GitHub 下载最新版本：
      </p>
      ...
      <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <a
          href="https://github.com/shadow1ng/fscan/releases/tag/1.8.4"
          target="_blank"
          class="flex items-center p-3 rounded-lg transition-all duration-200 border hover:-translate-y-1"
          :class="
            isDark
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              下载后直接运行可执行文件，无需安装其他依赖。Linux/MacOS 中需要执行权限 (<code
                >chmod +x fscan</code
              >)。
            </p>
          </div>
...
class="transition-colors duration-300"
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              ...
              <code
                ...
                :class="isDark ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-800'"
                >./fscan -h</code
              >
              命令可以查看完整的参数列表和帮助信息。
            </p>
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        ...
        <li>
          <router-link
            to="/documents/vulnerability-detection"
            class="transition-colors duration-200"
            :class="
              isDark ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'
            "
...
// 内容章节标题（用于目录）
const contentSections = ['下载与安装', '基本用法', '常用命令示例', '参数说明', '后续步骤']
