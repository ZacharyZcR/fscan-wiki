<template>
  <div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">MSSQL 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的 Microsoft SQL Server 数据库服务，并尝试通过弱密码进行身份验证。本指南详细介绍 MSSQL 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">Microsoft SQL Server（简称MSSQL）是微软公司推出的关系型数据库管理系统，广泛应用于企业环境中。Fscan 的 MSSQL 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              MSSQL 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border transition-colors duration-300"
          ...
          <div class="flex items-center mb-2">
            ...
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">结果收集</h3>
          </div>
          <p class="text-sm" :class="isDark ? 'text-gray-400' : 'text-gray-600'">
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in mssqlParameters" :key="param.name"
      ...
      <p class="mb-4">MSSQL 扫描模块使用 TDS 协议连接数据库并验证凭据。TDS（Tabular Data Stream）是 SQL Server 使用的专有通信协议。</p>
      ...
      <p class="mb-4">Fscan 使用 Go 语言的 <code>database/sql</code> 和 MSSQL 驱动来实现数据库连接：</p>
    ...
    case <-pingCtx.Done():
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">连接提示</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Fscan 使用的 <code>encrypt=disable</code> 参数仅适用于内网测试环境。在生产环境中，应启用加密以保护敏感信息的传输安全。
            </p>
          </div>
        </div>
      </div>
...
func tryMssqlCredential(ctx context.Context, info *Common.HostInfo, credential MssqlCredential,
    ...
    return &MssqlScanResult{
      ...
      <p class="mb-4">MSSQL 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 MSSQL 连接尝试的最大时间（秒）</li>
...
cancel()</code></pre>
      ...
      <p class="mb-4">通过实施这些安全措施，可以有效降低 MSSQL 服务面临的安全风险，提高数据库环境的整体安全性。</p>
    </div>
