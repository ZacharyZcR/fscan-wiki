  ...
  <div>
    ...
    <div class="flex items-center mb-6">
      <div class="w-10 h-10 flex-shrink-0 rounded-lg mr-3 flex items-center justify-center"
        ...
        <Icon icon="mdi:desktop-windows" class="text-xl" :class="isDark ? 'text-blue-400' : 'text-blue-600'" />
      </div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">RDP（Remote Desktop Protocol）扫描是 Fscan 的一个重要功能模块，用于发现网络中的远程桌面服务并尝试进行身份验证。本指南详细介绍 RDP 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">远程桌面协议（RDP）是微软开发的一种网络协议，允许用户连接到远程计算机并与之交互，就像坐在本地计算机前一样。Fscan 的 RDP 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              RDP 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行 RDP 扫描可能违反法律法规，并可能被视为入侵行为。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">RDP 扫描使用完整的 RDP 协议栈实现，包括 TPKT、X224、MCS、SEC 和 PDU 多个协议层，能够准确模拟真实 RDP 客户端的行为，提高扫描的准确性。</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in rdpParameters" :key="param.name"
      ...
      <div class="mb-6 relative overflow-hidden rounded-lg transition-colors duration-300 border"
           :class="isDark ? 'border-gray-700' : 'border-gray-200'">
        <div class="overflow-auto p-4"
             :class="isDark ? 'bg-gray-800' : 'bg-white'">
          <div class="flex flex-col items-stretch">
            <div class="p-3 border rounded-t-lg transition-colors duration-300"
                 :class="isDark ? 'bg-blue-900/20 border-blue-800/30' : 'bg-blue-50 border-blue-200'">
              <div class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">5. PDU 层</div>
              <div class="mt-1 text-sm" :class="isDark ? 'text-blue-200' : 'text-blue-600'">提供远程桌面基本图形接口</div>
            </div>
            <div class="p-3 border-l border-r transition-colors duration-300"
                 :class="isDark ? 'bg-blue-900/15 border-blue-800/30' : 'bg-blue-50/80 border-blue-200'">
              <div class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">4. SEC 层</div>
              <div class="mt-1 text-sm" :class="isDark ? 'text-blue-200' : 'text-blue-600'">处理 RDP 安全加密和身份认证</div>
            </div>
            <div class="p-3 border-l border-r transition-colors duration-300"
                 :class="isDark ? 'bg-blue-900/10 border-blue-800/30' : 'bg-blue-50/60 border-blue-200'">
              <div class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">3. MCS/T.125 层</div>
              <div class="mt-1 text-sm" :class="isDark ? 'text-blue-200' : 'text-blue-600'">处理多通道和数据传输</div>
            </div>
            <div class="p-3 border-l border-r transition-colors duration-300"
                 :class="isDark ? 'bg-blue-900/5 border-blue-800/30' : 'bg-blue-50/40 border-blue-200'">
              <div class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">2. X.224 层</div>
              <div class="mt-1 text-sm" :class="isDark ? 'text-blue-200' : 'text-blue-600'">处理连接初始化和通信控制</div>
            </div>
            <div class="p-3 border rounded-b-lg transition-colors duration-300"
                 :class="isDark ? 'bg-blue-900/0 border-blue-800/30' : 'bg-blue-50/20 border-blue-200'">
              <div class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">1. TPKT 层</div>
              <div class="mt-1 text-sm" :class="isDark ? 'text-blue-200' : 'text-blue-600'">处理基础网络传输和封包</div>
            </div>
          </div>
        </div>
      </div>

      <p class="mb-4">这种多层协议实现使 Fscan 能够处理各种 RDP 服务器配置，包括支持 NLA (Network Level Authentication) 的安全配置。</p>
    ...
    g.pdu.SetFastPathSender(g.tpkt)
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">NLA 支持说明</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              Fscan 支持通过 NTLMv2 进行 Network Level Authentication，这是一种更安全的 RDP 认证方式。NLA 要求在建立 RDP 会话之前进行用户认证，提高了安全性。
            </p>
          </div>
        </div>
      </div>
    ...
    g.pdu.On("done", func() {
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-green-300' : 'text-green-700'">安全考虑</p>
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              RDP 连接的异常处理机制确保了即使在网络不稳定或服务器过载的情况下，扫描也能正常结束而不会占用大量资源或导致程序崩溃。
            </p>
          </div>
        </div>
      </div>
...
func saveRdpResult(info *Common.HostInfo, target string, port int, credential RDPCredential) {
    ...
    Common.SaveResult(vulnResult)
      ...
      <p class="mb-4">这个配置使用 50 个线程，单个连接超时设置为 5 秒，全局扫描超时为 10 分钟，并将结果保存到文件。</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              使用 RDP 扫描时，建议从小范围开始测试并适当限制线程数和超时时间，避免对目标系统造成过大负载。在生产环境中进行扫描前，确保获得相关授权。
            </p>
          </div>
        </div>
      </div>
