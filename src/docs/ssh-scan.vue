<template>
  <div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">当未指定 SSH 密钥路径时，Fscan 默认使用密码爆破方式尝试认证：</p>
...
func attemptKeyAuth(info *Common.HostInfo, username, keyPath string, timeoutMs int64) (bool, error) {
    ...
    return true, nil
      ...
      <p class="mb-4">内部实现会自动根据线程数和凭据数量调整工作池大小：</p>
...
defer cancel()</code></pre>
      ...
      <p class="mb-4">这个配置使用 200 个线程，单个连接超时设置为 2 秒，并将结果保存到文件。</p>
    </div>
...
pre {
  position: relative;
  border-left: 3px solid rgba(149, 97, 216, 0.4);
}

.dark pre {
  border-left: 3px solid rgba(149, 97, 216, 0.6);
