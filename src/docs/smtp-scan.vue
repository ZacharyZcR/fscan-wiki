<template>
  <div>
    ...
    <div class="doc-content transition-colors duration-300"
      ...
      <p class="mb-4">SMTP 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的邮件服务器，并尝试检测匿名访问或通过弱密码进行身份验证。本指南详细介绍 SMTP 扫描的工作原理、配置选项和最佳实践。</p>
      ...
      <p class="mb-4">SMTP（Simple Mail Transfer Protocol）是用于发送电子邮件的标准协议，广泛应用于互联网和企业邮件系统。Fscan 的 SMTP 扫描模块可以：</p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              SMTP 扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规，也可能触发邮件服务器的安全防护机制。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="min-w-full rounded-lg overflow-hidden text-sm font-normal"
          ...
          <tr v-for="param in smtpParameters" :key="param.name"
...
if anonymousResult.Success {
    ...
    return nil
      ...
      <p class="mb-4">匿名访问检测会尝试不提供用户名和密码进行连接，如果服务器允许这种操作并且能够发送邮件，则表示存在安全风险。这通常是由于邮件服务器配置不当导致的。</p>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-amber-300' : 'text-amber-700'">安全隐患</p>
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              允许匿名访问的 SMTP 服务器可能被用于垃圾邮件发送或电子邮件欺骗攻击。在生产环境中，应确保 SMTP 服务器始终需要身份验证。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">SMTP 认证采用标准的 SMTP 协议流程，通过 Go 语言的 <code>net/smtp</code> 包实现：</p>
    ...
    return true, nil
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            <p class="font-medium" :class="isDark ? 'text-blue-300' : 'text-blue-700'">认证流程说明</p>
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              SMTP PLAIN 认证是一种基础认证方式，将凭据以 Base64 编码形式发送。现代 SMTP 服务器通常支持多种认证方式，如 LOGIN、CRAM-MD5 等，但 PLAIN 认证兼容性最好，被大多数服务器支持。
            </p>
          </div>
        </div>
      </div>
...
func concurrentSmtpScan(ctx context.Context, info *Common.HostInfo, credentials []SmtpCredential,
    ...
    // ...省略后续代码...
      ...
      <p class="mb-4">SMTP 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过 <code>-t</code> 参数设置：</p>
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 SMTP 连接尝试的最大时间（秒）</li>
...
conn.SetDeadline(time.Now().Add(timeout))</code></pre>
      ...
      <div class="p-3 rounded-lg mb-6 text-sm font-normal"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              SMTP 服务器可能会限制短时间内的连接尝试次数，防止暴力破解攻击。设置适当的超时和线程数可以避免触发服务器的防护机制。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">SMTP 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：</p>
...
func trySmtpCredential(ctx context.Context, info *Common.HostInfo, credential SmtpCredential,
    ...
    return &SmtpScanResult{Success: false, Error: lastErr, Credential: credential}
      ...
      <p class="mb-4">重试机制会跳过一些不需要重试的错误（如身份验证错误），只对网络波动等临时性问题进行重试，提高扫描效率。</p>
...
func saveSmtpResult(info *Common.HostInfo, target string, result *SmtpScanResult) {
    ...
    Common.SaveResult(vulnResult)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li><strong>匿名访问</strong>：标记为 "anonymous-access" 类型的漏洞，允许未经身份验证的用户访问</li>
        <li><strong>弱密码</strong>：标记为 "weak-password" 类型的漏洞，记录成功破解的用户名和密码</li>
      </ul>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              对 SMTP 服务进行扫描时，请控制好扫描速度和总请求量，避免触发目标服务器的安全防护机制。邮件服务器通常会限制来自同一 IP 的连接数和认证尝试次数。
            </p>
          </div>
        </div>
      </div>
...
const smtpParameters = [
  ...
  { name: '-user', description: '指定用户名列表，多个用逗号分隔', example: '-user admin,mail,postmaster' },
  { name: '-pwd', description: '指定密码列表，多个用逗号分隔', example: '-pwd 123456,{user}123,admin' },
