<template>
  <div>
    ...
    >
      <p class="mb-4">
        Modbus 扫描是 Fscan 的一项工业控制系统安全检测功能，用于发现网络中的 Modbus TCP
        设备并检测无认证访问风险。本指南详细介绍 Modbus 扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        Modbus
        是一种应用于工业控制系统的通信协议，广泛用于可编程逻辑控制器（PLC）、人机界面（HMI）、传感器和各类工业设备之间的通信。Fscan
        的 Modbus 扫描模块可以：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-orange-200' : 'text-orange-800'">
              Modbus
              协议设计时未考虑安全因素，对工业控制系统进行扫描可能影响设备正常运行。仅在授权的安全测试环境中使用此功能，避免对生产系统造成影响。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6">
        <h3
          class="text-lg font-semibold mb-3 transition-colors duration-300"
        ...
        <div class="overflow-auto">
          <div
            class="border rounded-lg transition-colors duration-300 p-2"
            :class="isDark ? 'border-gray-700' : 'border-gray-200'"
          >
            <div class="grid grid-cols-12 gap-1">
              <div
                class="col-span-2 p-2 text-center text-xs rounded"
              ...
              <div
                class="col-span-1 p-2 text-center text-xs rounded"
                :class="isDark ? 'bg-blue-900/20 text-blue-300' : 'bg-blue-100 text-blue-700'"
              >
                <div>单元标识符</div>
              ...
              <div
                class="col-span-1 p-2 text-center text-xs rounded"
                :class="
                  isDark ? 'bg-orange-900/20 text-orange-300' : 'bg-orange-100 text-orange-700'
                "
              >
                <div>功能码</div>
                <div class="font-mono mt-1">(1字节)</div>
              ...
              <div
                class="col-span-4 p-2 text-center text-xs rounded"
                :class="
                  isDark ? 'bg-orange-900/20 text-orange-300' : 'bg-orange-100 text-orange-700'
                "
              >
            ...
            <div class="grid grid-cols-2 gap-1 mt-1">
              <div
                class="p-1 text-center text-xs rounded"
              ...
              <div
                class="p-1 text-center text-xs rounded"
                :class="
                  isDark ? 'bg-orange-900/10 text-orange-300' : 'bg-orange-50 text-orange-600'
                "
              >
                PDU (协议数据单元)
              </div>
      ...
      <div
        class="mb-6 p-4 rounded-lg border transition-colors duration-300"
        :class="isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'"
      >
        <ol class="space-y-4">
          <li class="flex">
            <div
              class="w-6 h-6 rounded-md flex items-center justify-center mr-3"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              Modbus 协议中功能码 0x81 表示请求功能码 0x01 的错误响应。Modbus
              错误响应的功能码始终是请求功能码加上 0x80。
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in modbusParameters"
              :key="param.name"
...
for retry := 0; retry < maxRetries; retry++ {
    ...
    default:
      ...
      <p class="mb-4">
        通过这种方式，Fscan 实现了全局超时控制和单个连接超时控制，并在连接失败时提供了自动重试功能。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              工业网络环境通常响应较慢，建议适当增加超时时间，例如将默认的 3 秒增加到 5-10
              秒，以提高扫描的准确性。
            </p>
          </div>
        </div>
...
go func() {
    ...
    // ... 执行扫描逻辑 ...
      ...
      <p class="mb-4">
        通过 Goroutine 和通道机制，Fscan
        能够有效地控制并发扫描任务，同时确保不会因为单个扫描任务阻塞整个扫描过程。
      </p>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        Modbus 协议设计时未考虑安全性，因此在工业环境中使用 Modbus 需要采取额外的安全措施：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-red-200' : 'text-red-800'">
              未经保护的 Modbus
              设备可能被攻击者利用来监控或控制工业流程，甚至可能导致物理设备损坏或安全事故。定期进行安全评估和渗透测试对保障工业系统安全至关重要。
            </p>
          </div>
        </div>
      </div>
