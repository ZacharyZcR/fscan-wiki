<template>
  <div>
    ...
    >
      <p class="mb-4">
        SMB (Server Message Block) 扫描是 Fscan 的重要功能之一，用于发现网络中的 Windows
        文件共享服务，并尝试通过弱密码进行身份验证。本指南详细介绍 SMB
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        SMB 协议是 Windows
        网络环境中常用的文件共享协议，广泛应用于局域网内的文件访问、打印共享和域认证。Fscan 的 SMB
        扫描模块可以：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              SMB
              扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规，并可能触发账户锁定策略。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        SMB
        扫描的实现考虑了网络环境的安全性，通过错误检测和账户锁定检测，降低对目标系统的影响。同时通过并发扫描和超时控制，提高了扫描效率。
      </p>
      ...
      <p class="mb-4">
        SMB 服务通常配置有账户锁定策略，多次错误密码尝试会导致账户被锁定。Fscan 的 SMB
        扫描模块通过智能识别账户锁定状态，避免继续尝试已锁定的账户：
      </p>
...
if result.Error != nil && strings.Contains(result.Error.Error(), "账号锁定") {
    ...
    Common.LogError(fmt.Sprintf("用户 %s 已被锁定", credential.Username))
      ...
      <p class="mb-4">
        该机制通过维护一个锁定用户列表，在扫描过程中动态识别和记录被锁定的账户，避免无效尝试，降低对目标系统的影响。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              在扫描生产环境时，建议使用较小的密码列表，并降低线程数（如 -t
              5），减少触发账户锁定策略的风险。
            </p>
          </div>
        </div>
      ...
      <p class="mb-4">
        SMB 认证使用 NTLM 或 Kerberos 协议进行身份验证，Fscan 通过调用底层 SMB 库实现连接和认证：
      </p>
...
func SmblConn(info *Common.HostInfo, user string, pass string, signal chan struct{}) (flag bool, err error) {
    ...
    return false, err
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              SMB认证使用会话安全协议，支持NTLM和Kerberos认证机制。在域环境中，通过设置 -domain
              参数进行域身份验证。
            </p>
          </div>
        </div>
      ...
      <p class="mb-4">
        SMB 扫描使用工作池模式进行并发扫描，提高扫描效率的同时避免对目标系统造成过大压力：
      </p>
...
if Common.Domain != "" {
    ...
    successMsg = fmt.Sprintf("SMB认证成功 %s %s:%s", target, credential.Username, credential.Password)
      ...
      <p class="mb-4">
        在域环境中，通过 <code>-domain</code> 参数指定域名，格式为 <code>DOMAIN\\username</code> 或
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        该命令使用低并发（3线程）、较长超时（5秒）进行扫描，减少对目标系统的影响，并将结果保存到文件。
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-green-200' : 'text-green-800'">
              由于 Windows
              环境通常具有账户锁定策略，建议在扫描前了解目标环境的安全策略，使用精简的密码列表，避免触发大量账户锁定。
            </p>
          </div>
        </div>
      </div>
...
const smbParameters = [
  ...
  },
  {
    ...
    example: '-pwd admin123,{user}@2023',
  },
