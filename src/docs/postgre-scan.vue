<template>
  <div>
    ...
    >
      <p class="mb-4">
        PostgreSQL 扫描是 Fscan 的重要功能模块，用于发现网络中运行的 PostgreSQL
        数据库服务，并尝试通过弱密码进行身份验证。本指南详细介绍 PostgreSQL
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        PostgreSQL 是一个功能强大的开源关系型数据库系统，以其可靠性、数据完整性和正确性而闻名。Fscan
        的 PostgreSQL 扫描模块可以：
      </p>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in postgresParameters"
              :key="param.name"
      ...
      <p class="mb-4">
        PostgreSQL 扫描模块使用标准数据库驱动和 SQL
        查询来验证凭据。连接过程包括建立连接、身份验证以及权限验证三个阶段。
      </p>
      ...
      <p class="mb-4">
        PostgreSQL 连接使用标准 Go SQL 驱动与 <code>lib/pq</code> 库，通过连接字符串指定连接参数：
      </p>
...
func PostgresConn(ctx context.Context, info *Common.HostInfo, user string, pass string) (bool, error) {
    ...
    return true, nil
      ...
      <p class="mb-4">
        成功连接后，通过执行 <code>SELECT version()</code> 查询验证用户权限并获取数据库版本信息：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              PostgreSQL
              中的权限管理非常精细。即使连接成功，用户可能只拥有有限的权限。通过尝试执行简单查询可以进一步验证用户的实际权限级别。
            </p>
          </div>
        </div>
      </div>
...
for _, user := range Common.Userdict["postgresql"] {
    for _, pass := range Common.Passwords {
      ...
      <p class="mb-4">
        支持密码变量替换功能，例如
        <code>{user}</code> 会被替换为当前尝试的用户名，便于生成常见的密码模式。
      </p>
...
func tryPostgresCredential(ctx context.Context, info *Common.HostInfo, credential PostgresCredential,
    ...
    return &PostgresScanResult{Success: false, Error: lastErr, Credential: credential}
      ...
      <p class="mb-4">
        PostgreSQL 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 PostgreSQL
          连接尝试的最大时间（秒）
        </li>
        <li>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              在扫描大型网络时，合理设置超时参数非常重要。PostgreSQL
              的默认连接超时通常较长，适当缩短可以提高扫描效率。
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              PostgreSQL 的默认用户通常是
              "postgres"，将其作为首要尝试的用户名可以提高扫描效率。此外，使用 "{user}"
              变量可以生成与用户名相关的密码模式，这是常见的弱密码设置方式。
            </p>
          </div>
        </div>
      </div>
