<template>
  <div>
    ...
    >
      <p class="mb-4">
        RabbitMQ 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的 RabbitMQ
        消息代理服务，并尝试通过弱密码进行身份验证。本指南详细介绍 RabbitMQ
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        RabbitMQ
        是一个开源的消息代理软件，实现了高级消息队列协议（AMQP），被广泛应用于分布式系统集成、微服务架构和云原生应用。Fscan
        的 RabbitMQ 扫描模块可以：
      </p>
      ...
      <div class="overflow-x-auto mb-6">
        ...
        >
          ...
          <tbody class="divide-y" :class="isDark ? 'divide-gray-700' : 'divide-gray-200'">
            <tr
              v-for="param in rabbitmqParameters"
              :key="param.name"
      ...
      <p class="mb-4">
        RabbitMQ 扫描模块使用 AMQP（高级消息队列协议）来验证凭据。AMQP 是 RabbitMQ
        的核心协议，提供了一种标准化的方式来发送和接收消息。
      </p>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-purple-200' : 'text-purple-800'">
              RabbitMQ 默认安装时会创建 guest/guest 账户，但从 3.3.0
              版本开始，该账户仅允许从本地主机连接。这是一项重要的安全特性，但在某些配置中可能被更改。
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              当连接失败时，RabbitMQ 会返回具体的错误信息，例如 "ACCESS_REFUSED" 表示凭据错误。Fscan
              会根据错误类型决定是否需要重试，提高扫描效率。
            </p>
          </div>
        </div>
      </div>
...
func tryRabbitMQCredential(ctx context.Context, info *Common.HostInfo, credential RabbitMQCredential,
    ...
    // ...省略部分代码...
        ...
        >
          <div class="flex items-center mb-2">
            <Icon
              ...
              class="text-lg mr-2"
              :class="isDark ? 'text-yellow-400' : 'text-yellow-500'"
            />
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">
              超时错误
            </h3>
        ...
        >
          <div class="flex items-center mb-2">
            <Icon
              ...
              class="text-lg mr-2"
              :class="isDark ? 'text-orange-400' : 'text-orange-500'"
            />
            <h3 class="font-medium" :class="isDark ? 'text-gray-200' : 'text-gray-800'">
              连接错误
            </h3>
      ...
      <p class="mb-4">
        RabbitMQ 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
func concurrentRabbitMQScan(ctx context.Context, info *Common.HostInfo, credentials []RabbitMQCredential,
    ...
    // ...省略部分代码...
      ...
      <p class="mb-4">
        内部实现使用协程池并发处理多个凭据检测任务，一旦发现有效凭据会立即取消其他正在进行的任务，提高扫描效率。
      </p>
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 RabbitMQ
          连接尝试的最大时间（秒）
        </li>
        <li>
...
cancel()</code></pre>
      ...
      <p class="mb-4">
        Fscan 的 RabbitMQ 扫描模块支持智能重试机制，通过
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-purple-200' : 'text-purple-800'">
              使用 RabbitMQ 时应始终更改默认凭据，使用强密码，并限制允许访问的 IP
              地址。保证消息队列安全性是分布式系统安全的重要组成部分。
            </p>
          </div>
        </div>
      </div>
...
const rabbitmqParameters = [
  ...
  {
    name: '-p 5672,15672',
    description: 'RabbitMQ 常用端口，多个用逗号分隔',
    example: '-p 5672,15672',
  },
