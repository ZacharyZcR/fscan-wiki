<template>
  <div>
    ...
    >
      <p class="mb-4">
        IMAP 扫描是 Fscan 的一个重要功能模块，用于发现网络中运行的 IMAP
        邮件服务器，并尝试通过弱密码进行身份验证。本指南详细介绍 IMAP
        扫描的工作原理、配置选项和最佳实践。
      </p>
      ...
      <p class="mb-4">
        ...
        Protocol）是一种用于访问电子邮件的协议，允许用户从远程邮件服务器上访问和管理邮件。Fscan 的
        IMAP 扫描模块可以：
      </p>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-blue-200' : 'text-blue-800'">
              IMAP
              扫描功能应当仅用于合法的安全测试和授权的渗透测试。未经授权对系统进行扫描可能违反法律法规，邮件服务尤其敏感。
            </p>
          </div>
        </div>
      </div>
      ...
      <p class="mb-4">
        IMAP 扫描模块使用标准的 IMAP 协议命令来验证凭据。认证过程支持普通连接和 TLS 加密连接：
      </p>
...
func tryIMAPAuth(conn net.Conn, user string, pass string, timeout time.Duration) (bool, error) {
    ...
    for {
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          命令格式: <code>a001 LOGIN "username" "password"</code>，其中 a001 是客户端定义的标签
        </li>
        ...
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              现代 IMAP 服务器通常会提供多种认证机制，但 Fscan 目前使用最基本的 LOGIN
              命令进行认证测试。这种方法适用于大多数服务器，但某些高安全性服务器可能禁用基本认证。
            </p>
          </div>
        </div>
      </div>
...
func concurrentIMAPScan(ctx context.Context, info *Common.HostInfo, credentials []IMAPCredential,
    ...
    // ...省略发送工作和等待结果部分...
      ...
      <p class="mb-4">
        IMAP 扫描模块使用 <code>ModuleThreadNum</code> 参数控制并发线程数，可以通过
        <code>-t</code> 参数设置：
      </p>
...
if maxConcurrent > len(credentials) {
    maxConcurrent = len(credentials)
      ...
      <ul class="pl-5 list-disc space-y-2 mb-6">
        <li>
          <strong>单个连接超时</strong>：通过 <code>-time</code> 参数设置每个 IMAP
          连接尝试的最大时间（秒）
        </li>
        <li>
...
conn.SetDeadline(time.Now().Add(timeout))</code></pre>
      ...
      >
        <div class="flex">
          ...
          <div>
            ...
            <p class="mt-1" :class="isDark ? 'text-amber-200' : 'text-amber-800'">
              IMAP
              服务器响应时间可能较长，特别是在高负载的邮件服务器上。建议将单个连接超时设置为至少 5
              秒，以避免因超时过短而导致的误报。
            </p>
          </div>
        </div>
      ...
      <p class="mb-4">
        Fscan 的 IMAP 扫描模块支持自动重试连接，通过 <code>-retries</code> 参数设置最大重试次数：
      </p>
...
class="transition-colors duration-300"
      ...
      <p class="mb-4">
        这个配置使用 50 个线程，单个连接超时设置为 5 秒，最大重试次数为 2，并将结果保存到文件。
      </p>
...
const imapParameters = [
  ...
  {
    name: '-p 143,993',
    description: 'IMAP 常用端口，多个用逗号分隔（143为普通IMAP，993为IMAPS）',
    example: '-p 143,993',
  },
  {
    name: '-user',
    description: '指定用户名列表，多个用逗号分隔',
    example: '-user admin,user1,user2',
  },
  {
    name: '-pwd',
    description: '指定密码列表，多个用逗号分隔',
    example: '-pwd admin123,{user}@123',
  },
